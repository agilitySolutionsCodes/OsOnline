...
#End Region
    ...
    Public Function SelecionarAnexos(ByVal numero As String) As SqlDataReader
        Dim conn = ctlUtil.GetConnection
        Dim cmd As New SqlCommand("PR_LISTAR_ITENS_ANEXADOS_OS", conn)
    ...
    Public Function ValidarNomeAnexo(ByVal nomeArquivo As String) As SqlDataReader
        Dim conn = ctlUtil.GetConnection
        Dim cmd As New SqlCommand("PR_LISTAR_ANEXOS_OS", conn)

        cmd.CommandTimeout = conn.ConnectionTimeout
        cmd.CommandType = CommandType.StoredProcedure

        cmd.Parameters.Add(New SqlParameter("@P_FILIAL", ctlUtil.GetFilial))
        cmd.Parameters.Add(New SqlParameter("@P_NOMEARQUIVO", nomeArquivo))
    ...
    Public Function Incluir(ByVal oFicha As ctlOrdemServico) As ctlRetornoGenerico
        ...
        Dim oAnexosD As New List(Of wsmicrosiga.ordemservico.ANEXOORDEMSERVICOSTRUCT)
        For Each oAnexo As ctlAnexo In oFicha.Anexos
            Dim oAnexoD As New wsmicrosiga.ordemservico.ANEXOORDEMSERVICOSTRUCT
            oAnexoD = DirectCast(oAnexo.PutSiga(oAnexoD), wsmicrosiga.ordemservico.ANEXOORDEMSERVICOSTRUCT)

            oAnexosD.Add(oAnexoD)
        Next

        oFichaSiga.ANEXOS = oAnexosD.ToArray
